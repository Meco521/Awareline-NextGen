...
/*  60 */             bool1 = m & "MC|PingHost".equals(new String(bytebuf.readBytes(bytebuf.readShort() << 1).array(), Charsets.UTF_16BE));
/*  61 */             j = bytebuf.readUnsignedShort();
/*  62 */             k = bool1 & ((bytebuf.readUnsignedByte() >= 73) ? 1 : 0);
/*  63 */             k &= (3 + (bytebuf.readBytes(bytebuf.readShort() << 1).array()).length + 4 == j) ? 1 : 0;
